  ...
  head: () => ({
    meta: [
      { title: "Produits — Pharmavert" },
      {
        name: "description",
        content:
          "Découvrez les 4 produits phares Pharmavert : Déconat, Cumax, Nobal et Ociamag — formules naturelles pour les professionnels de santé.",
      },
      { property: "og:title", content: "Nos Produits — Pharmavert" },
      {
        property: "og:description",
        content: "Compléments alimentaires naturels développés par Pharmavert.",
      },
        ...
        <div className="pv-container text-center">
          <Reveal>
            ...
            <p className="text-lg text-muted-foreground max-w-3xl mx-auto">
              Découvrez les 4 produits phares Pharmavert — des formules naturelles et bioactives
              développées pour les professionnels de santé et disponibles dans votre réseau de
        ...
        <div className="pv-container text-center max-w-3xl mx-auto">
          <Reveal>
            ...
            <p className="text-white/85 mb-8">
              Contactez notre équipe commerciale pour recevoir notre catalogue complet et discuter
              d'un partenariat de distribution.
            </p>
...
function ProductSection({
  ...
  return (
    <section
      id={product.id}
      className={`section-pad scroll-mt-24 ${alt ? "bg-surface" : "bg-white"}`}
    >
          ...
          <Reveal delay={0.05}>
            ...
            {product.composition && (
              <div className="grid sm:grid-cols-3 gap-3 mb-7">
                {product.composition.map((c) => (
                  <div
                    key={c.name}
                    className="bg-white border border-border rounded-xl p-4 border-t-[3px] border-t-primary"
                  >
            ...
            <div className="flex flex-wrap gap-2 mb-8">
              {product.ingredients.map((i) => (
                <span
                  key={i}
                  className="text-xs font-bold px-3 py-1.5 rounded-full bg-card-accent text-primary border border-primary/20"
                >
                  {i}
                </span>
              ))}
